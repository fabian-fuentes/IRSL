...
################################################################################
#        PROGRAMA DE CÁLCULO DEL ÍNDICE DE REZAGO SOCIAL LONGITUDINAL          # 
#             AL INTERIOR DE LAS ENTIDADES FEDERATIVAS, 2000-2020              #
################################################################################
...
# Variables utilizadas para la construcción del Índice de Rezago Social Longitudinal al interior de las entidades
...
files <-list.files('bases/ITER_INEGI',full.names = TRUE)
...
iter <- lapply(files, function(f){
  ...
  # Se homologan las variables que serán utilizadas para el cálculo en función 
  # de las especificidades de cada periodo
  ...
  # Se calculan las variables que serán empleadas para la construcción del Índice de Rezago Social Longitudinal al interior de las entidades
  ...
  df[,anio:=as.numeric(paste0('20',gsub("[^0-9.-]", "", f)))]
  
  # Se define el nivel de agregación del registro a partir de las claves de localidad, 
  # municipio y entidad
  
  df <- df[!loc %in% c(9999,9998,0)]
...
dt_tot <- rbindlist(iter)
dt_tot <- dt_tot[order(anio,decreasing = T)]

ent<-unique(dt_tot$cve_ent)


# Se realiza la estimación para las 32 entidades federativas

tbs <- lapply(ent, function(x){

  print(paste0('Inicia la estimación para la entidad:',x))
  ...
  # para escalar los datos con unidades comparables, de esta forma el calculo del ACP 
  ...
  # Calculo del ACP por año, se ponderan los pesos para sumar 1
  ...
  #######################################################################################
  # 2. Construcción del Índice de Rezago Social Longitudinal al interior de las entidades
  #######################################################################################
  ...
  # Se calcula la media promedio a partir de la media observada por año, para centrar 
  # los datos con unidades comparables, de esta forma el calculo del IRSL (posterior al ACP) 
  # partirá del mismo punto para cada observación
  ...
  ##############################################################################
  # 3. Grado de Rezago Social Longitudinal al interior de las entidades
  ##############################################################################
  
  # Cálculo del Grado de Rezago Social Longitudinal al interior de las entidades 
  # a través del método de Dalenius y Hodges, utilizando los registros de cada entidad federativa
  
  # Se realizan las iteraciones con la información de la entidad
  
  dh <- lapply(5:200, function(cl){
    
    print(paste0('Iteración:',cl))
    dh <- suppressWarnings(strata.cumrootf(as.numeric(pc_sianre_val$irsl), CV = 0.05, Ls = 3, nclass = cl)$stratumID) %>% data.table()
 ...
 print(paste0('Termina entidad:', x))
...
fwrite(rbindlist(tbs), 'bases_finales/IRSL_interior_entidades.csv', bom=T)
